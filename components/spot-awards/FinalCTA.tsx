  ...
  return (
    <section className="relative bg-dark-300 overflow-hidden">
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse at 80% 80%, rgba(29,97,246,0.18) 0%, rgba(237,123,48,0.08) 40%, transparent 70%)",
        }}
      />
      {/* Cool glow — top left */}
      <div
        className="absolute -top-20 -left-20 w-[360px] h-[360px] pointer-events-none"
        style={{
          background:
            "radial-gradient(circle, rgba(29,97,246,0.1) 0%, transparent 65%)",
        }}
      />
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-10 lg:gap-16">
          ...
          >
            <h2 className="text-4xl lg:text-5xl font-bold tracking-tight leading-tight mb-6">
              <span className="text-white">Start recognizing your<br /></span>
            ...
            <p className="text-dark-000 text-base leading-relaxed">
              Book a 20-minute walkthrough. We&apos;ll show you exactly how recognition can become a daily habit at your company.
            </p>
          </motion.div>

          {/* Right: CTAs + compliance */}
          <motion.div
            className="flex flex-col gap-4 lg:items-end lg:shrink-0"
            initial={reduce ? undefined : { opacity: 0, y: 24 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, amount: 0.4 }}
            transition={{ duration: 0.55, ease: [0, 0, 0.2, 1] as const, delay: 0.1 }}
          >
            {/* Buttons */}
            <div className="flex items-center gap-5">
              <a
                href="#"
                className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white text-dark-300 font-bold text-sm hover:bg-light-200 transition-colors shadow-lg"
              >
                Book a demo
              ...
              <a
                href="#"
                className="inline-flex items-center gap-1.5 text-white text-sm font-semibold hover:text-blue-100 transition-colors"
              >
                Talk to sales
            ...
            <div className="flex items-center gap-2 flex-wrap lg:justify-end">
              {COMPLIANCE.map((c, i) => (
                <span key={c} className="flex items-center gap-2">
                  <span className="text-[10px] font-bold uppercase tracking-[0.12em] text-dark-000/60">
                    {c}
                  </span>
                  {i < COMPLIANCE.length - 1 && (
                    <span className="text-dark-000/25 text-xs">·</span>
                  )}
                </span>
              ))}
            </div>
