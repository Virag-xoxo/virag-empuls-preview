...
const FLOATING_CHIPS = [
  { id: 1, emoji: "🏆", label: "6 challenge templates", sub: "Steps · minutes · distance",   value: "All",   pos: "top-[6%] right-[2%]",     delay: "0s",   val: "text-blue-200"   },
  { id: 2, emoji: "📲", label: "6 trackers synced",      sub: "Apple · Fitbit · Strava · more", value: "Live",  pos: "top-[58%] right-[-2%]",   delay: "0.6s", val: "text-green-300"  },
  { id: 3, emoji: "🎁", label: "Milestone rewards",      sub: "Auto-credited to wallet",       value: "Auto",  pos: "bottom-[10%] right-[6%]", delay: "1.1s", val: "text-orange-200" },
];
...
const TABS = [
  { e: "👟", l: "Stepathon",  active: true  },
  { e: "💪", l: "Workout",    active: false },
  { e: "🏃", l: "5K Run",     active: false },
  { e: "🏃‍♀️", l: "10K Run",     active: false },
  { e: "🏅", l: "Marathon",   active: false },
];

const RANKS = [
  { i: "SM", name: "Sarah M.",  steps: 12840, you: false },
  { i: "DT", name: "Daniel T.", steps: 11205, you: false },
  { i: "MH", name: "Megan H.",  steps: 10418, you: true  },
];
...
export default function Hero() {
  ...
  useEffect(() => {
    const t = setInterval(() => setActive((p) => (p + 1) % RANKS.length), 1800);
  ...
  return (
    <section className="relative flex flex-col overflow-hidden bg-dark-300">
      ...
      <div className="blob-3 absolute bottom-0 left-1/3 w-[450px] h-[450px] rounded-full pointer-events-none" style={{ background: "radial-gradient(circle, rgba(5,150,105,0.16) 0%, transparent 65%)" }} />
      ...
      <div className="relative z-10 w-full max-w-[1280px] mx-auto px-6 pt-28 pb-12 grid lg:grid-cols-[1fr_500px] gap-16 items-center">
        <div className="flex flex-col items-start">
          ...
          <motion.h1 {...fadeUp(0.1)} className="text-[46px] lg:text-[60px] font-bold text-white leading-[1.06] tracking-tight mb-6">
            <span className="block">Make wellness</span>
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-100 to-blue-200">a team sport</span>
          </motion.h1>

          <motion.p {...fadeUp(0.2)} className="text-lg text-dark-000 max-w-md mb-10 leading-relaxed">
            Live leaderboards, milestone rewards, and six tracker integrations — all inside Empuls.
          </motion.p>
        ...
        <motion.div className="relative hidden lg:block h-[540px]"
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[360px] bg-white/10 backdrop-blur-xl rounded-2xl border border-white/15 p-5 shadow-2xl">
            <div className="flex items-start justify-between mb-3 pb-3 border-b border-white/10">
              <div>
                <p className="text-white text-[12px] font-bold leading-tight">Stepathon · April</p>
                <p className="text-dark-000 text-[10px] mt-0.5">41 participants · Monthly reset</p>
              </div>
            ...
            <div className="rounded-xl mb-3 overflow-hidden h-[88px] relative" style={{ background: "linear-gradient(135deg,#10B981 0%,#059669 50%,#047857 100%)" }}>
              ...
              <div className="absolute bottom-2 left-3 text-white">
                <p className="text-[9px] font-bold uppercase tracking-[0.10em] text-white/80">Today &middot; live sync</p>
                <p className="text-[12px] font-bold leading-tight">10,418 steps logged</p>
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              {RANKS.map((r, i) => (
                <motion.div key={r.i}
                  ...
                  <span className="text-[10px] font-bold text-blue-100 tabular-nums">{r.steps.toLocaleString()}</span>
                </motion.div>
              ))}
            </div>
            ...
            <div className="mt-3 pt-3 border-t border-white/10 flex items-center justify-between">
              <div>
                <p className="text-[9px] uppercase tracking-[0.10em] text-white/55 font-bold">Next milestone</p>
                <p className="text-[10px] text-white tabular-nums">10,500 → 500 pts</p>
              </div>
              <button className="text-[10px] font-bold text-white bg-blue-200 px-3 py-1.5 rounded-md">View roadmap</button>
            </div>
          </div>
