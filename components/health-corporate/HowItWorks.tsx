...
function SetupModal() {
  ...
  useEffect(() => {
    if (reduce) return;
    const t = setInterval(() => setActive((p) => (p + 1) % 4), 1300);
    return () => clearInterval(t);
  }, [reduce]);
  const items = [
    { e: "🩺", l: "Preventive checkups",   on: true  },
    { e: "👨‍⚕️", l: "Doctor consultations",   on: true  },
    { e: "🛡️", l: "Group health insurance", on: true  },
    { e: "💊", l: "Pharmacy delivery",      on: false },
  ];
  return (
    <div className="bg-white rounded-2xl shadow-xl border border-light-200 overflow-hidden w-full">
      <div className="px-5 py-3.5 border-b border-light-200 flex items-center gap-2">
        ...
        <p className="text-sm font-bold text-dark-300">Programs &gt; Manage Benefits &gt; Wellness</p>
      </div>
      <div className="px-5 py-4 space-y-2">
        {items.map((c, i) => (
          <motion.div key={c.l}
            ...
            <span className={`text-[9px] font-bold uppercase tracking-[0.10em] ${c.on ? "text-green-300" : "text-dark-100"}`}>{c.on ? "Active" : "Off"}</span>
          </motion.div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl border border-light-200 overflow-hidden w-full">
      <div className="px-5 py-3.5 border-b border-light-200 flex items-center gap-2">
        ...
        <p className="text-sm font-bold text-dark-300">Choose partner &middot; Health checkup</p>
      </div>
      <div className="px-5 py-4 space-y-2">
        {[
          { e: "🧪", l: "Healthians",  sub: "At-home full body checkup",  pkg: "₹2,499 / emp" },
          { e: "👨‍⚕️", l: "Practo",       sub: "OPD + 25+ specialties",       pkg: "From ₹1,200" },
          { e: "🛡️", l: "Pazcare",     sub: "Group health insurance",      pkg: "₹5L–₹15L" },
        ].map((it) => (
          <div key={it.l} className="flex items-center gap-3 rounded-xl px-3 py-2.5 bg-light-100 border border-light-200">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[11px] font-bold text-dark-300 truncate">{it.l}</p>
              <p className="text-[10px] text-dark-100 truncate">{it.sub}</p>
            </div>
            <span className="text-[10px] font-bold text-blue-200 tabular-nums">{it.pkg}</span>
          </div>
        ))}
      </div>
    </div>
...
function RedeemModal() {
  ...
  const events = [
    { e: "📅", l: "Slot booked",         sub: "Mon · 9:30 AM · doorstep",  done: true },
    { e: "🧪", l: "Sample collected",    sub: "Phlebotomist on the way",   done: step >= 1 },
    { e: "📄", l: "Digital report sent", sub: "Delivered to employee app", done: step >= 2 },
  ];
  return (
    <div className="bg-white rounded-2xl shadow-xl border border-light-200 overflow-hidden w-full">
      <div className="px-5 py-3.5 border-b border-light-200 flex items-center gap-2">
        <span className="text-base">✨</span>
        <p className="text-sm font-bold text-dark-300">Annual checkup &middot; live</p>
      </div>
      <div className="px-5 py-4 space-y-2">
        {events.map((ev, i) => (
...
const STEPS = [
  { num: "01", tag: "Set up",   title: "Configure the program",     body: "Admins log in and navigate to Programs &gt; Manage Benefits &gt; Wellness Benefits to switch on the modules they want.", cta: "About setup",     Illustration: SetupModal },
  { num: "02", tag: "Choose",   title: "Pick partners and packages", body: "Select from Healthians, Practo, Pazcare, ekincare and more. Pricing and coverage tiers are surfaced inline.",            cta: "About partners",  Illustration: PartnersModal },
  { num: "03", tag: "Redeem",   title: "Employees book and use",     body: "Employees access wellness through Shop &gt; Health &amp; Wellness, book at-home tests, and get digital reports.",         cta: "About redemption", Illustration: RedeemModal },
];
    ...
    <section className="relative py-20 lg:py-28 overflow-hidden" style={{ background: "linear-gradient(160deg, #ffffff 0%, #f4f7ff 55%, #eef3ff 100%)" }}>
      ...
      <div className="relative z-10 max-w-[1280px] mx-auto px-6">
        <div className="text-center max-w-xl mx-auto mb-14">
          ...
          <h2 className="text-3xl lg:text-4xl font-bold text-dark-300 tracking-tight mb-4">Up and running in three steps</h2>
          <p className="text-dark-100 text-base leading-relaxed">Configure modules, choose partners, and let employees book — admins launch in minutes, not weeks.</p>
        </div>
