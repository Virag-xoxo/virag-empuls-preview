...
const FLOATING_CHIPS = [
  { id: 1, emoji: "🩺", label: "At-home diagnostics",   sub: "Doorstep · digital reports",     value: "Live",  pos: "top-[6%] right-[2%]",     delay: "0s",   val: "text-green-300"  },
  { id: 2, emoji: "👨‍⚕️", label: "25+ specialties",        sub: "Practo telehealth network",       value: "All",   pos: "top-[58%] right-[-2%]",   delay: "0.6s", val: "text-blue-200"   },
  { id: 3, emoji: "🛡️", label: "Group insurance",        sub: "₹5L to ₹15L sum insured",         value: "₹15L",  pos: "bottom-[10%] right-[6%]", delay: "1.1s", val: "text-orange-200" },
];
...
const TABS = [
  { e: "🩺", l: "Checkups",   active: true  },
  { e: "👨‍⚕️", l: "Doctors",    active: false },
  { e: "🛡️", l: "Insurance",  active: false },
  { e: "💊", l: "Pharmacy",   active: false },
  { e: "🧠", l: "Mental",     active: false },
];

const PROGRAMS = [
  { name: "Healthians",  type: "Full body checkup",   detail: "At-home", active: true  },
  { name: "Practo",       type: "Doctor consultation", detail: "Telehealth", active: false },
  { name: "Pazcare",     type: "Group health insurance", detail: "₹10L plan",  active: false },
];
...
export default function Hero() {
  ...
  useEffect(() => {
    const t = setInterval(() => setActive((p) => (p + 1) % PROGRAMS.length), 1800);
    ...
    <section className="relative flex flex-col overflow-hidden bg-dark-300">
      ...
      <div className="relative z-10 w-full max-w-[1280px] mx-auto px-6 pt-28 pb-12 grid lg:grid-cols-[1fr_500px] gap-16 items-center">
        <div className="flex flex-col items-start">
          ...
          <motion.h1 {...fadeUp(0.1)} className="text-[46px] lg:text-[60px] font-bold text-white leading-[1.06] tracking-tight mb-6">
            <span className="block">Complete health and</span>
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-100 to-blue-200">wellness for your team</span>
          </motion.h1>

          <motion.p {...fadeUp(0.2)} className="text-lg text-dark-000 max-w-md mb-10 leading-relaxed">
            Preventive checkups, digital doctor access, and group health insurance — one platform employees already use.
          </motion.p>
        ...
        <motion.div className="relative hidden lg:block h-[540px]"
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[360px] bg-white/10 backdrop-blur-xl rounded-2xl border border-white/15 p-5 shadow-2xl">
            <div className="flex items-start justify-between mb-3 pb-3 border-b border-white/10">
              <div>
                <p className="text-white text-[12px] font-bold leading-tight">Wellness Benefits</p>
                <p className="text-dark-000 text-[10px] mt-0.5">3 active programs · 5 categories</p>
              </div>
              <span className="text-[9px] font-bold text-green-300 bg-green-300/15 border border-green-300/30 px-2 py-0.5 rounded-full whitespace-nowrap">Live</span>
            </div>

            <div className="flex items-center gap-1 overflow-x-auto pb-1 mb-3">
              {TABS.map((t) => (
                <span key={t.l} className={`text-[9px] font-bold whitespace-nowrap shrink-0 px-2 py-1 rounded-full inline-flex items-center gap-1 ${t.active ? "bg-blue-200 text-white" : "bg-white/8 text-white/65"}`}>
                  <span>{t.e}</span>{t.l}
                </span>
              ))}
            </div>

            <div className="rounded-xl mb-3 overflow-hidden h-[88px] relative" style={{ background: "linear-gradient(135deg,#10B981 0%,#059669 50%,#047857 100%)" }}>
              <div className="absolute inset-0 flex items-center justify-center text-3xl">🩺</div>
              <div className="absolute bottom-2 left-3 text-white">
                <p className="text-[9px] font-bold uppercase tracking-[0.10em] text-white/80">Checkups · active</p>
                <p className="text-[12px] font-bold leading-tight">Annual full body screening</p>
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              {PROGRAMS.map((p, i) => (
                <motion.div key={p.name}
                  ...
                  className={`flex items-center gap-2 rounded-lg px-2.5 py-1.5 border ${active === i ? "bg-green-300/15 border-green-300/30" : "bg-white/5 border-white/10"}`}>
                  <div className="flex-1 min-w-0">
                    <p className="text-[11px] font-bold text-white truncate">{p.name}</p>
                    <p className="text-[9px] text-white/55 truncate">{p.type}</p>
                  </div>
                  <span className="text-[10px] font-bold text-blue-100 tabular-nums">{p.detail}</span>
                </motion.div>
              ))}
            </div>
            ...
            <div className="mt-3 pt-3 border-t border-white/10 flex items-center justify-between">
              <div>
                <p className="text-[9px] uppercase tracking-[0.10em] text-white/55 font-bold">Healthians · Annual</p>
                <p className="text-[10px] text-white tabular-nums">Doorstep collection</p>
              </div>
              <button className="text-[10px] font-bold text-white bg-blue-200 px-3 py-1.5 rounded-md">Schedule</button>
            </div>
          </div>
      ...
      <motion.div className="relative z-10 w-full pt-4 pb-8" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: reduce ? 0 : 0.6, ease, delay: 0.55 }}>
        <div className="flex items-center gap-5 max-w-[1280px] mx-auto px-6 mb-5">
          ...
          <p className="text-[10px] text-dark-100 uppercase tracking-[0.18em] font-semibold whitespace-nowrap">Trusted by 5,000+ teams worldwide</p>
          <div className="flex-1 h-px bg-white/10" />
        </div>
