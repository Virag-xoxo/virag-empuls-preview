...
const FLOATING_CHIPS = [
  { id: 1, emoji: "🎯", label: "7 benefit types",      sub: "in a single platform",      value: "All",   pos: "top-[6%] right-[2%]",     delay: "0s",   val: "text-blue-200" },
  { id: 2, emoji: "💰", label: "Tax + company-funded", sub: "From allowances to LSAs",   value: "Mix",   pos: "top-[58%] right-[-2%]",   delay: "0.6s", val: "text-orange-200" },
  { id: 3, emoji: "✨", label: "Choice, not mandate",  sub: "Employees pick what fits",  value: "✓",     pos: "bottom-[10%] right-[6%]", delay: "1.1s", val: "text-green-300" },
];
...
const BENEFITS = [
  { e: "💰", label: "Tax-saving",       sub: "Meals · Fuel · Telecom",  state: "Active" },
  { e: "🪐", label: "Lifestyle",        sub: "Spending account",         state: "Active" },
  { e: "🩺", label: "Health & wellness", sub: "Checkups · Mental health", state: "Active" },
  { e: "🛍️", label: "Discounts",        sub: "Perks store · Cashback",   state: "Available" },
  { e: "💸", label: "Early wages",       sub: "Earned-wage access",       state: "Available" },
  { e: "🛡️", label: "Insurance",         sub: "Group health · Life",      state: "Available" },
  { e: "💻", label: "Devices",           sub: "Laptops · Phones · EMI",   state: "Available" },
];
      ...
      <div className="relative z-10 w-full max-w-[1280px] mx-auto px-6 pt-28 pb-12 grid lg:grid-cols-[1fr_500px] gap-16 items-center">
        <div className="flex flex-col items-start">
          ...
          <motion.h1 {...fadeUp(0.1)} className="text-[46px] lg:text-[60px] font-bold text-white leading-[1.06] tracking-tight mb-6">
            <span className="block">Benefits tailored</span>
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-100 to-blue-200">to every employee</span>
          </motion.h1>

          <motion.p {...fadeUp(0.2)} className="text-lg text-dark-000 max-w-md mb-10 leading-relaxed">
            Perks and benefits that cover every sphere of life — personal, financial, physical, and mental.
          </motion.p>
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[360px] bg-white/10 backdrop-blur-xl rounded-2xl border border-white/15 p-5 shadow-2xl">
            <div className="flex items-start justify-between mb-4 pb-4 border-b border-white/10">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <p className="text-white text-[12px] font-bold leading-tight">My benefits portal</p>
                  <p className="text-dark-000 text-[10px] mt-0.5">7 categories · sync from HRMS</p>
                </div>
              </div>
              <span className="inline-flex items-center gap-1 bg-green-200/15 border border-green-200/20 rounded-full px-2 py-0.5 text-green-300 text-[9px] font-bold whitespace-nowrap">
                <span className="w-1 h-1 rounded-full bg-green-300 animate-pulse" />ENROLLED
              </span>
            </div>

            <div className="space-y-1.5">
              {BENEFITS.map((b) => (
                <div key={b.label} className="flex items-center gap-2.5 bg-white/5 border border-white/10 rounded-lg px-2.5 py-1.5">
                  <span className="text-base shrink-0">{b.e}</span>
                  <div className="flex-1 min-w-0">
                    <p className="text-[11px] font-bold text-white truncate">{b.label}</p>
                    <p className="text-[9px] text-white/50 truncate">{b.sub}</p>
                  </div>
                  <span className={`text-[8px] font-bold uppercase tracking-[0.06em] px-1.5 py-0.5 rounded-full whitespace-nowrap ${b.state === "Active" ? "bg-blue-200/30 text-blue-100" : "bg-white/10 text-white/55"}`}>{b.state}</span>
                </div>
              ))}
            </div>

            <div className="mt-3 pt-3 border-t border-white/10 flex items-center justify-between">
              <span className="text-[9px] text-white/55">Synced from Workday · auto-enrolled</span>
              <span className="text-[9px] font-bold text-blue-100">Manage →</span>
            </div>
          </div>
