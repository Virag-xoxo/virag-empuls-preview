...
const FLOATING_CHIPS = [
  { id: 1, emoji: "📋", label: "Multiple categories",  sub: "Meal · Fuel · Phone · LTA",  value: "Mix",   pos: "top-[6%] right-[2%]",     delay: "0s",   val: "text-blue-200" },
  { id: 2, emoji: "✅", label: "100% tax compliant",   sub: "Auto-calculated on claims",  value: "Auto",  pos: "top-[58%] right-[-2%]",   delay: "0.6s", val: "text-green-300" },
  { id: 3, emoji: "📲", label: "Zero paper claims",    sub: "Receipt upload to payroll", value: "0",     pos: "bottom-[10%] right-[6%]", delay: "1.1s", val: "text-orange-200" },
];
...
const CATEGORIES = [
  { e: "🍽️", label: "Meal allowance",        sub: "$220 / month",  state: "Active" },
  { e: "⛽", label: "Fuel reimbursement",    sub: "$160 / month",  state: "Active" },
  { e: "📱", label: "Phone & internet",      sub: "$100 / month",  state: "Add" },
  { e: "✈️", label: "Travel allowance",      sub: "Biennial · LTA",state: "Active" },
];
      ...
      <div className="relative z-10 w-full max-w-[1280px] mx-auto px-6 pt-28 pb-12 grid lg:grid-cols-[1fr_500px] gap-16 items-center">
        <div className="flex flex-col items-start">
          ...
          <motion.h1 {...fadeUp(0.1)} className="text-[46px] lg:text-[60px] font-bold text-white leading-[1.06] tracking-tight mb-6">
            <span className="block">Flexible benefits</span>
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-100 to-blue-200">for every employee</span>
          </motion.h1>
        ...
        <motion.div className="relative hidden lg:block h-[540px]"
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[360px] bg-white/10 backdrop-blur-xl rounded-2xl border border-white/15 p-5 shadow-2xl">
            <div className="flex items-start justify-between mb-3 pb-3 border-b border-white/10">
              <div>
                <p className="text-white text-[12px] font-bold leading-tight">My flexible benefits</p>
                <p className="text-dark-000 text-[10px] mt-0.5">FY 2025–26 · annual budget</p>
              </div>
              <span className="text-[9px] font-bold text-blue-100 bg-blue-200/20 border border-blue-100/30 px-2 py-0.5 rounded-full whitespace-nowrap">$5,000</span>
            </div>

            <div className="mb-3">
              <div className="flex items-center justify-between text-[10px] mb-1">
                <span className="text-white/55">Allocated so far</span>
                <span className="text-white font-bold tabular-nums">$3,840 (76%)</span>
              </div>
              <div className="h-2 bg-white/8 rounded-full overflow-hidden">
                <div className="h-full rounded-full bg-gradient-to-r from-blue-100 to-blue-200" style={{ width: "76%" }} />
              </div>
              <p className="text-[9px] text-white/45 mt-1">$3,840 allocated · $1,160 remaining</p>
            </div>

            <div className="space-y-1.5">
              {CATEGORIES.map((c) => (
                <div key={c.label} className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-lg px-2.5 py-1.5">
                  <span className="text-base shrink-0">{c.e}</span>
                  <div className="flex-1 min-w-0">
                    <p className="text-[11px] font-bold text-white truncate">{c.label}</p>
                    <p className="text-[9px] text-white/50 truncate">{c.sub}</p>
                  </div>
                  <span className={`text-[8px] font-bold uppercase tracking-[0.06em] px-1.5 py-0.5 rounded-full whitespace-nowrap ${c.state === "Active" ? "bg-green-300/20 text-green-300" : "bg-blue-200/20 text-blue-100"}`}>{c.state === "Add" ? "+ ADD" : c.state}</span>
                </div>
              ))}
            </div>

            <div className="mt-3 pt-3 border-t border-white/10 flex items-center justify-between">
              <span className="text-[9px] text-white/55">2 more categories available</span>
              <span className="text-[9px] font-bold text-blue-100">Manage plan →</span>
            </div>
          </div>
