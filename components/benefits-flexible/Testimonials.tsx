...
const FEATURED = {
  ...
  metric: "76",
  metricSuffix: "%",
  metricLabel: "budget allocation rate",
  metricContext: "of eligible employees fully allocated their flexible benefits budget within 30 days of plan launch",
  quote: "We replaced a 12-page benefits enrollment PDF with a portal employees actually open. People allocate their budget on day one, not at year-end. The math just runs.",
  name: "Catherine Reed",
...
const COMPACT = [
  {
    ...
    metric: "92",
    metricSuffix: "%",
    metricLabel: "claims processed digitally",
    metricContext: "of all benefit claims filed through the app — receipts uploaded, approved, and reimbursed without paper",
    quote: "Before Empuls, our finance team was processing reimbursement spreadsheets for two days every cycle. Now claims close the same day they're filed.",
    name: "Daniel Foster",
  ...
  },
  {
    ...
    metricContext: "average tax savings per employee from properly structured pre-tax allowances",
    quote: "The take-home calculator at the moment of allocation is the killer feature. People see exactly what they save by using the plan — and they actually use it.",
    name: "Megan Hayes",
...
const STATS = [
  { value: "76%",   label: "Budget allocated" },
  { value: "92%",   label: "Claims digital" },
  { value: "$1.8K", label: "Avg tax savings" },
...
function AnimatedStat({ value, label }: { value: string; label: string }) {
  ...
  const suffix = value.replace(/[0-9.$K]/g, "");
  const count = useCountUp(numeric, 1200, visible && !reduce);
  const display = isCurrency ? `$${count.toFixed(1)}${isK ? "K" : ""}` : `${Math.floor(count)}${suffix}`;
  ...
  return (
    <div ref={ref} className="text-center">
      <p className="text-4xl lg:text-5xl font-bold text-white mb-1 tabular-nums">{reduce ? value : display}</p>
    ...
    <section className="relative bg-dark-300 py-20 lg:py-28 overflow-hidden">
      ...
      <div className="max-w-[1280px] mx-auto px-6">
        <div className="text-center max-w-xl mx-auto mb-14">
          ...
          <h2 className="text-3xl lg:text-4xl font-bold text-white tracking-tight mb-4">People teams that retired the benefits PDF</h2>
          <p className="text-dark-000 text-base leading-relaxed">Companies that swapped paper enrollment forms for a portal employees actually open — and saw allocation, claims speed, and tax savings improve together.</p>
        </div>
