...
const FLOATING_CHIPS = [
  { id: 1, emoji: "🎯", label: "9 engagement drivers",  sub: "tracked per survey",       value: "All",   pos: "top-[6%] right-[2%]",     delay: "0s",   val: "text-blue-200" },
  { id: 2, emoji: "⚙️", label: "Fully automated",       sub: "Zero manual sending",      value: "✓",     pos: "top-[58%] right-[-2%]",   delay: "0.6s", val: "text-green-300" },
  { id: 3, emoji: "📈", label: "Trended over time",     sub: "By department, location",  value: "6mo",   pos: "bottom-[10%] right-[6%]", delay: "1.1s", val: "text-orange-200" },
];
...
export default function Hero() {
  ...
  useEffect(() => {
    if (reduce) return;
    const t = setInterval(() => setSelectedRating((p) => (p === 4 ? 3 : 4)), 2200);
      ...
      <div className="relative z-10 w-full max-w-[1280px] mx-auto px-6 pt-28 pb-12 grid lg:grid-cols-[1fr_500px] gap-16 items-center">
        <div className="flex flex-col items-start">
          ...
          <motion.h1 {...fadeUp(0.1)} className="text-[46px] lg:text-[60px] font-bold text-white leading-[1.06] tracking-tight mb-6">
            <span className="block">Know how your team</span>
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-100 to-blue-200">feels every day.</span>
          </motion.h1>
        ...
        <motion.div className="relative hidden lg:block h-[540px]"
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[340px] bg-white/10 backdrop-blur-xl rounded-2xl border border-white/15 p-5 shadow-2xl">
            ...
            <div className="flex items-center justify-between mb-2">
              <span className="text-[10px] font-bold uppercase tracking-[0.10em] text-blue-100 font-mono">Q1 of 5</span>
              <span className="text-[9px] font-bold text-white/70 bg-white/8 px-2 py-0.5 rounded-full">Work-life balance</span>
            </div>
            <p className="text-[13px] font-bold text-white leading-snug mb-3">How satisfied are you with your workload this week?</p>

            <div className="flex items-center justify-between gap-1.5 mb-4">
              {moods.map((m, i) => (
                <button key={i}
                  onClick={() => setSelectedRating(i)}
                  className={`flex-1 aspect-square rounded-lg flex items-center justify-center text-xl transition-all ${selectedRating === i ? "bg-blue-200/30 border-2 border-blue-100" : "bg-white/5 border border-white/10 hover:bg-white/10"}`}>
                  {m}
                </button>
              ))}
            </div>
            ...
            <div className="flex items-center justify-between mb-2">
              <span className="text-[10px] text-white/55">1 of 5 questions</span>
              <span className="text-[10px] font-bold text-blue-100">Next →</span>
            </div>
            <div className="h-1 bg-white/8 rounded-full overflow-hidden mb-3">
              <div className="h-full rounded-full bg-blue-200" style={{ width: "20%" }} />
            </div>

            <div className="pt-3 border-t border-white/10 flex items-center justify-between text-[10px]">
              <span className="text-white/55">📊 157 of 263 responded so far</span>
              <span className="text-green-300 font-bold">↑ +5% vs last week</span>
            </div>
          </div>
