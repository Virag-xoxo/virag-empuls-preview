...
const FLOATING_CHIPS = [
  { id: 1, emoji: "🪐", label: "5 wellness categories", sub: "Fitness · checkups · pharmacy", value: "All",   pos: "top-[6%] right-[2%]",     delay: "0s",   val: "text-blue-200" },
  { id: 2, emoji: "🏢", label: "Bulk or points-based",   sub: "Employee or employer-funded", value: "Mix",   pos: "top-[58%] right-[-2%]",   delay: "0.6s", val: "text-orange-200" },
  { id: 3, emoji: "🎁", label: "Wallet redemption",      sub: "Same wallet as rewards",     value: "Live",   pos: "bottom-[10%] right-[6%]", delay: "1.1s", val: "text-green-300" },
];
...
const TABS = [
  { e: "🏋️", l: "Fitness",     active: true },
  { e: "🩺", l: "Checkups",    active: false },
  { e: "💊", l: "Pharmacy",    active: false },
  { e: "🧠", l: "Mental",      active: false },
  { e: "👨‍👩‍👧", l: "Family",      active: false },
];

const PROVIDERS = [
  { name: "CultFit",  type: "Gym & yoga",       pts: 1200, active: true  },
  { name: "Apollo",   type: "Health checkup",   pts:  900, active: false },
  { name: "Mfine",    type: "Mental health",    pts:  600, active: false },
];
...
export default function Hero() {
  ...
  useEffect(() => {
    if (reduce) return;
    const t = setInterval(() => setActive((p) => (p + 1) % PROVIDERS.length), 1800);
      ...
      <div className="relative z-10 w-full max-w-[1280px] mx-auto px-6 pt-28 pb-12 grid lg:grid-cols-[1fr_500px] gap-16 items-center">
        <div className="flex flex-col items-start">
          ...
          <motion.h1 {...fadeUp(0.1)} className="text-[46px] lg:text-[60px] font-bold text-white leading-[1.06] tracking-tight mb-6">
            <span className="block">Promote wellbeing</span>
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-100 to-blue-200">across the workplace</span>
          </motion.h1>

          <motion.p {...fadeUp(0.2)} className="text-lg text-dark-000 max-w-md mb-10 leading-relaxed">
            Fitness, health checkups, pharmacy, mental health, and care for loved ones — one platform, one wallet.
          </motion.p>
        ...
        <motion.div className="relative hidden lg:block h-[540px]"
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[360px] bg-white/10 backdrop-blur-xl rounded-2xl border border-white/15 p-5 shadow-2xl">
            <div className="flex items-start justify-between mb-3 pb-3 border-b border-white/10">
              <div>
                <p className="text-white text-[12px] font-bold leading-tight">Wellness Store</p>
                <p className="text-dark-000 text-[10px] mt-0.5">Balance · 2,400 pts available</p>
              </div>
              <span className="text-[9px] font-bold text-green-300 bg-green-300/15 border border-green-300/30 px-2 py-0.5 rounded-full whitespace-nowrap">5 categories</span>
            </div>
            ...
            <div className="rounded-xl mb-3 overflow-hidden h-[88px] relative" style={{ background: "linear-gradient(135deg,#10B981 0%,#059669 50%,#047857 100%)" }}>
              ...
              <div className="absolute bottom-2 left-3 text-white">
                <p className="text-[9px] font-bold uppercase tracking-[0.10em] text-white/80">Fitness · active</p>
                <p className="text-[12px] font-bold leading-tight">Move, sweat, recover</p>
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              {PROVIDERS.map((p, i) => (
                <motion.div key={p.name}
                  ...
                  <span className="text-[10px] font-bold text-blue-100 tabular-nums">{p.pts.toLocaleString()} pts</span>
                </motion.div>
              ))}
            </div>
            ...
            <div className="mt-3 pt-3 border-t border-white/10 flex items-center justify-between">
              <div>
                <p className="text-[9px] uppercase tracking-[0.10em] text-white/55 font-bold">CultFit Premium</p>
                <p className="text-[10px] text-white tabular-nums">1,200 pts</p>
              </div>
              <button className="text-[10px] font-bold text-white bg-blue-200 px-3 py-1.5 rounded-md">Redeem now</button>
            </div>
          </div>
