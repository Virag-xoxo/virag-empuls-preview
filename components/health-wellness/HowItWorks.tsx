...
function BrowseModal() {
  ...
  useEffect(() => {
    if (reduce) return;
    const t = setInterval(() => setActive((p) => (p + 1) % 5), 1300);
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl border border-light-200 overflow-hidden w-full">
      <div className="px-5 py-3.5 border-b border-light-200 flex items-center gap-2">
        ...
        <p className="text-sm font-bold text-dark-300">Redeem &gt; Health &amp; Wellness</p>
      </div>
      <div className="px-5 py-4 grid grid-cols-2 gap-2">
        {cats.map((c, i) => (
          <motion.div key={c.l}
            ...
            <span className="text-[11px] font-bold text-dark-300 truncate">{c.l}</span>
          </motion.div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl border border-light-200 overflow-hidden w-full">
      <div className="px-5 py-3.5 border-b border-light-200 flex items-center gap-2">
        ...
        <p className="text-sm font-bold text-dark-300">Pick a category &middot; Fitness</p>
      </div>
      <div className="px-5 py-4 space-y-2">
        {[
          { e: "🧘", l: "CultFit Premium",     pts: "1,200 pts" },
          { e: "🏃", l: "ClassPass · 4 weeks", pts: "1,000 pts" },
          { e: "🚴", l: "Peloton · 1 month",   pts: "1,400 pts" },
        ].map((it) => (
          <div key={it.l} className="flex items-center gap-3 rounded-xl px-3 py-2.5 bg-light-100 border border-light-200">
            ...
            <span className="text-[10px] font-bold text-blue-200 tabular-nums">{it.pts}</span>
          </div>
        ))}
      </div>
    </div>
...
function RedeemModal() {
  ...
  const events = [
    { e: "🪙", l: "Points debited",      sub: "1,200 pts · CultFit Premium",  done: true },
    { e: "📧", l: "Voucher emailed",     sub: "Activation in 30 seconds",     done: step >= 1 },
    { e: "🎉", l: "Booked &amp; ready",  sub: "Confirmation in app",          done: step >= 2 },
  ];
  return (
    <div className="bg-white rounded-2xl shadow-xl border border-light-200 overflow-hidden w-full">
      <div className="px-5 py-3.5 border-b border-light-200 flex items-center gap-2">
        <span className="text-base">✨</span>
        <p className="text-sm font-bold text-dark-300">Redemption &middot; live</p>
      </div>
      <div className="px-5 py-4 space-y-2">
        {events.map((ev, i) => (
          <motion.div key={i} animate={{ opacity: ev.done ? 1 : 0.4 }} transition={{ duration: 0.25 }}
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[11px] font-bold text-dark-300" dangerouslySetInnerHTML={{ __html: ev.l }} />
...
const STEPS = [
  { num: "01", tag: "Browse",  title: "Open the Wellness Store",     body: "Employees navigate to Redeem &gt; Health &amp; Wellness in the Empuls app or web. No separate provider account needed.",        cta: "About browsing",   Illustration: BrowseModal },
  { num: "02", tag: "Choose",  title: "Pick a category and offer",   body: "Five categories cover fitness, health checkups, pharmacy, mental health, and care for loved ones. Pick what fits the moment.", cta: "About categories", Illustration: ChooseModal },
  { num: "03", tag: "Redeem",  title: "Book with points or pack",    body: "Pay with reward points, or use a company-sponsored bulk pack. Voucher and confirmation arrive instantly.",                       cta: "About redemption", Illustration: RedeemModal },
];
    ...
    <section className="relative py-20 lg:py-28 overflow-hidden" style={{ background: "linear-gradient(160deg, #ffffff 0%, #f4f7ff 55%, #eef3ff 100%)" }}>
      ...
      <div className="relative z-10 max-w-[1280px] mx-auto px-6">
        <div className="text-center max-w-xl mx-auto mb-14">
          ...
          <h2 className="text-3xl lg:text-4xl font-bold text-dark-300 tracking-tight mb-4">From wellness store to instant redemption</h2>
          <p className="text-dark-100 text-base leading-relaxed">Five wellness categories, one wallet — points or company-sponsored packs both work side by side.</p>
        </div>
