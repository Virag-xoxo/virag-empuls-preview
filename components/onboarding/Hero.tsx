...
const FLOATING_CHIPS = [
  { id: 1, emoji: "📦", label: "Welcome kit shipped",  sub: "Branded swag · Day 1",       value: "Live",  pos: "top-[6%] right-[2%]",     delay: "0s",   bg: "bg-white border-light-200", val: "text-blue-200" },
  { id: 2, emoji: "⭐", label: "500 welcome points",   sub: "Credited automatically",     value: "Sent",  pos: "top-[58%] right-[-2%]",   delay: "0.6s", bg: "bg-white border-light-200", val: "text-orange-200" },
  { id: 3, emoji: "🎉", label: "Team shoutout live",   sub: "12 reactions in 1 hour",     value: "Buzz",  pos: "bottom-[10%] right-[6%]", delay: "1.1s", bg: "bg-white border-light-200", val: "text-green-300" },
];
      ...
      <div className="relative z-10 w-full max-w-[1280px] mx-auto px-6 pt-28 pb-12 grid lg:grid-cols-[1fr_500px] gap-16 items-center">
        <div className="flex flex-col items-start">
          ...
          <motion.h1 {...fadeUp(0.1)} className="text-[46px] lg:text-[60px] font-bold text-white leading-[1.06] tracking-tight mb-6">
            <span className="block">Make day one feel</span>
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-100 to-blue-200">like a real beginning</span>
          </motion.h1>

          <motion.p {...fadeUp(0.2)} className="text-lg text-dark-000 max-w-md mb-10 leading-relaxed">
            Branded welcome kits, gift points, and team shoutouts — triggered the moment they join, automatically.
          </motion.p>
        ...
        <motion.div className="relative hidden lg:block h-[540px]"
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-72 bg-white/10 backdrop-blur-xl rounded-2xl border border-white/15 p-5 shadow-2xl">
            <div className="inline-flex items-center gap-1.5 bg-blue-200/15 border border-blue-100/20 rounded-full px-2.5 py-1 mb-3">
              ...
              <span className="text-blue-100 text-[10px] font-semibold uppercase tracking-wide">New Joiner · Auto-triggered</span>
            </div>

            <div className="flex items-center gap-3 mb-4 pb-4 border-b border-white/10">
              <div className="w-9 h-9 rounded-full shrink-0 flex items-center justify-center text-white text-xs font-bold bg-blue-200">JB</div>
              <div className="min-w-0 flex-1">
                <p className="text-white text-xs font-semibold">Jordan Brooks</p>
                <p className="text-dark-000 text-[10px]">Software Engineer · Joins today</p>
              </div>
              <span className="shrink-0 bg-orange-200 text-dark-300 text-[9px] font-extrabold rounded-full px-2 py-0.5 tracking-wide">DAY 1</span>
            </div>

            <p className="text-white/80 text-[11px] leading-relaxed mb-3 italic">
              &ldquo;Welcome aboard, Jordan! We&apos;re thrilled to have you here. Your welcome kit is ready — dive in whenever you&apos;re set up.&rdquo;
            </p>

            <p className="text-dark-000 text-[10px] uppercase tracking-wider font-semibold mb-2">Welcome kit includes</p>
            <div className="grid grid-cols-3 gap-1 mb-3">
              <div className="bg-white/8 border border-white/10 rounded-lg px-2 py-2 text-center">
                <p className="text-base mb-0.5">📦</p>
                <p className="text-[9px] text-white font-semibold">Branded Swag</p>
              </div>
              <div className="bg-white/8 border border-white/10 rounded-lg px-2 py-2 text-center">
                <p className="text-base mb-0.5">⭐</p>
                <p className="text-[9px] text-white font-semibold">500 Points</p>
              </div>
              <div className="bg-white/8 border border-white/10 rounded-lg px-2 py-2 text-center">
                <p className="text-base mb-0.5">💌</p>
                <p className="text-[9px] text-white font-semibold">Welcome Note</p>
              </div>
            </div>

            <div className="flex items-center gap-1.5 pt-3 border-t border-white/10">
              <span className="text-dark-000 text-[10px]">Sent via</span>
              <span className="text-white/85 text-[10px] bg-white/8 border border-white/10 rounded px-1.5 py-0.5 font-semibold">Slack</span>
              <span className="text-white/85 text-[10px] bg-white/8 border border-white/10 rounded px-1.5 py-0.5 font-semibold">Teams</span>
              <span className="text-white/85 text-[10px] bg-white/8 border border-white/10 rounded px-1.5 py-0.5 font-semibold">Email</span>
            </div>
          </div>
