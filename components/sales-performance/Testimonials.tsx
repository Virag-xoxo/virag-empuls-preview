...
const FEATURED = {
  company: "ServiceNow",
  industry: "Enterprise SaaS · 22,000 employees",
  metric: "90",
  metricSuffix: "%",
  metricLabel: "increase in operational efficiency",
  metricContext: "from retiring manual quota tracking and replacing four spreadsheets with one live dashboard",
  quote: "Our managers used to spend Tuesday mornings rebuilding the leaderboard. Now they spend Tuesday mornings coaching the deals at risk &mdash; the data is already in front of them.",
  name: "Daniel Foster",
...
const COMPACT = [
  {
    company: "Atlassian",
    industry: "SaaS · 13,000 employees",
    metric: "100",
    metricSuffix: "%",
    metricLabel: "payout accuracy",
    metricContext: "across all programs since automatic calculation replaced the comp-comp spreadsheet",
    quote: "We&apos;ve had zero payout errors in twelve months. The number reps see is the number they get. The fact that we ever ran this on a spreadsheet now feels insane.",
    name: "Catherine Reed",
    title: "Sales Compensation Lead",
    photo: "https://randomuser.me/api/portraits/women/45.jpg",
  },
  {
    company: "Datadog",
    industry: "Observability · 7,000 employees",
    metric: "10",
    metricSuffix: "×",
    metricLabel: "faster plan launch",
    metricContext: "vs the spreadsheet-and-template cycle. New plans live in an afternoon, not a sprint",
    quote: "Plan iteration used to take two weeks. With the no-code designer, we shipped three plan variants in an afternoon and ran A/B tests across the AE pods within a sprint.",
    name: "Megan Hayes",
...
const STATS = [
  ...
  { value: "10×",  label: "Faster plan launch" },
  { value: "175+", label: "Payout countries" },
];
...
function AnimatedStat({ value, label }: { value: string; label: string }) {
  ...
  const display = Math.floor(count).toString();
    ...
    <section className="relative bg-dark-300 py-20 lg:py-28 overflow-hidden">
      ...
      <div className="max-w-[1280px] mx-auto px-6">
        <div className="text-center max-w-xl mx-auto mb-14">
          ...
          <h2 className="text-3xl lg:text-4xl font-bold text-white tracking-tight mb-4">Sales orgs that turned dashboards into a habit</h2>
          <p className="text-dark-000 text-base leading-relaxed">Companies that replaced manual reports, monthly reviews, and end-of-quarter scrambles with a live performance system reps actually check.</p>
        </div>
        ...
        <motion.figure className="bg-white/8 border border-white/12 rounded-2xl p-7 lg:p-8 mb-4 grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-6 lg:gap-12 items-center"
          ...
          <div>
            ...
            <blockquote className="text-dark-000 text-base leading-relaxed pl-4 border-l-2 border-blue-200/40" dangerouslySetInnerHTML={{ __html: `&ldquo;${FEATURED.quote}&rdquo;` }} />
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-16">
          {COMPACT.map((t, i) => (
            <motion.figure key={t.company} className="bg-white/5 border border-white/10 rounded-2xl p-6"
              ...
              <blockquote className="text-dark-000 text-sm leading-relaxed mb-5" dangerouslySetInnerHTML={{ __html: `&ldquo;${t.quote}&rdquo;` }} />
