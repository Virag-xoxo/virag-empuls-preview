...
const FLOATING_CHIPS = [
  { id: 1, emoji: "🎉", label: "6 lifecycle touchpoints", sub: "Onboarding to exit",      value: "All",    pos: "top-[6%] right-[2%]",     delay: "0s",   val: "text-blue-200" },
  { id: 2, emoji: "🔄", label: "HRIS-triggered",          sub: "Zero manual sending",     value: "Auto",   pos: "top-[58%] right-[-2%]",   delay: "0.6s", val: "text-green-300" },
  { id: 3, emoji: "📚", label: "Templates · DEIB to perf",sub: "Versioned per cohort",    value: "Custom", pos: "bottom-[10%] right-[6%]", delay: "1.1s", val: "text-orange-200" },
];
...
const RATINGS = [
  { l: "Onboarding experience", stars: 5, val: "5.0", state: "done" as const },
  { l: "Role clarity",           stars: 4, val: "4.0", state: "done" as const },
  { l: "Tools & resources",      stars: 5, val: "5.0", state: "done" as const },
  { l: "Team integration",       stars: 0, val: "answering...", state: "active" as const },
];
...
function StarRow({ rating }: { rating: number }) {
  return (
    <div className="flex items-center gap-0.5">
      {[1,2,3,4,5].map((s) => (
        <span key={s} className={`text-[11px] ${rating >= s ? "text-blue-100" : "text-white/20"}`}>★</span>
      ))}
    </div>
  );
}
      ...
      <div className="relative z-10 w-full max-w-[1280px] mx-auto px-6 pt-28 pb-12 grid lg:grid-cols-[1fr_500px] gap-16 items-center">
        <div className="flex flex-col items-start">
          ...
          <motion.p {...fadeUp(0.2)} className="text-lg text-dark-000 max-w-md mb-10 leading-relaxed">
            From first day to last &mdash; lifecycle surveys trigger automatically at each milestone so you never miss a critical employee touchpoint again.
          </motion.p>
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[340px] bg-white/10 backdrop-blur-xl rounded-2xl border border-white/15 p-5 shadow-2xl">
            <div className="flex items-start justify-between mb-4 pb-4 border-b border-white/10">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <p className="text-white text-[12px] font-bold leading-tight">Lifecycle Trigger · Onboarding</p>
                  <p className="text-dark-000 text-[10px] mt-0.5">Day 30 milestone reached · Jun 18, 2024</p>
                </div>
              </div>
              <span className="inline-flex items-center gap-1 bg-green-200/15 border border-green-200/20 rounded-full px-2 py-0.5 text-green-300 text-[9px] font-bold whitespace-nowrap">✓ AUTO-SENT</span>
            </div>

            <div className="flex items-center gap-3 mb-3">
              <div className="w-9 h-9 rounded-full flex items-center justify-center text-[10px] font-bold text-white shrink-0" style={{ background: "linear-gradient(135deg,#1D61F6,#6f8eff)" }}>SC</div>
              <div className="flex-1 min-w-0">
                <p className="text-white text-[12px] font-bold truncate">Sarah Chen</p>
                <p className="text-white/55 text-[10px] truncate">Design Team · Joined May 19, 2024</p>
              </div>
            </div>

            <div className="flex items-center gap-2 mb-3">
              <span className="text-[10px] uppercase tracking-[0.10em] text-white/55">Survey triggered</span>
              <span className="text-[9px] font-bold text-green-300 bg-green-300/15 px-2 py-0.5 rounded-full">30-DAY CHECK-IN</span>
            </div>

            <p className="text-[10px] uppercase tracking-[0.10em] text-white/55 font-bold mb-2">Responses so far</p>
            <div className="space-y-1.5 mb-4">
              {RATINGS.map((r) => (
                <div key={r.l} className={`flex items-center gap-2 rounded-lg px-2.5 py-1.5 ${r.state === "active" ? "bg-blue-200/15 border border-blue-100/30" : "bg-white/5 border border-white/10"}`}>
                  <p className="flex-1 text-[10px] font-bold text-white truncate">{r.l}</p>
                  {r.state === "done" ? <StarRow rating={r.stars} /> : null}
                  <span className={`text-[10px] font-bold tabular-nums w-12 text-right ${r.state === "active" ? "text-blue-100 italic font-mono" : "text-white"}`}>{r.val}</span>
                </div>
              ))}
            </div>
            ...
            <div className="mb-2">
              <div className="flex items-center justify-between text-[10px] mb-1">
                <span className="text-white/55">Survey progress</span>
                <span className="text-white font-mono font-bold">8 of 10 questions</span>
              </div>
              <div className="h-1.5 bg-white/8 rounded-full overflow-hidden">
                <div className="h-full rounded-full bg-gradient-to-r from-blue-100 to-blue-200" style={{ width: "80%" }} />
              </div>
            </div>

            <div className="pt-3 border-t border-white/10 flex items-center justify-between">
              <span className="text-[9px] text-white/55">Workday · Triggered automatically</span>
              <span className="text-[9px] font-bold text-blue-100 font-mono">ZERO MANUAL ACTION</span>
            </div>
          </div>
