...
const FLOATING_CHIPS = [
  { id: 1, emoji: "🎁", label: "6,000+ gift cards",   sub: "Save 5–50%",                value: "Live", pos: "top-[6%] right-[2%]",     delay: "0s",   val: "text-blue-200" },
  { id: 2, emoji: "🛍️", label: "20,000+ brands",      sub: "Across 25+ categories",     value: "Mix",  pos: "top-[58%] right-[-2%]",   delay: "0.6s", val: "text-orange-200" },
  { id: 3, emoji: "💰", label: "$7,000+ saved",       sub: "Avg per household / yr",     value: "Free", pos: "bottom-[10%] right-[6%]", delay: "1.1s", val: "text-green-300" },
];
...
const CATEGORIES = ["All", "Food & Dining", "Travel", "Wellness", "Shopping"];

const BRANDS = [
  { name: "Amazon",      tag: "Gift card",    save: "15%", color: "linear-gradient(135deg,#FF9900,#FFB347)" },
  { name: "Starbucks",   tag: "Gift card",    save: "18%", color: "linear-gradient(135deg,#006241,#00754A)" },
  { name: "Airbnb",      tag: "Travel",       save: "20%", color: "linear-gradient(135deg,#FF5A5F,#FF8E91)" },
  { name: "Netflix",     tag: "Streaming",    save: "10%", color: "linear-gradient(135deg,#E50914,#F40612)" },
  { name: "Uber",        tag: "Rideshare",    save: "8%",  color: "linear-gradient(135deg,#000,#333)" },
  { name: "+5,995",      tag: "more brands",  save: "",    color: "linear-gradient(135deg,#1D61F6,#6f8eff)" },
];
        ...
        <div className="flex flex-col items-start">
          <motion.div {...fadeUp(0)}>
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/8 border border-white/12 text-blue-100 text-xs font-semibold mb-8 backdrop-blur-sm">
              ...
              Employee Perks Store
            </div>
          </motion.div>

          <motion.h1 {...fadeUp(0.1)} className="text-[46px] lg:text-[60px] font-bold text-white leading-[1.06] tracking-tight mb-6">
            <span className="block">Discounts that</span>
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-100 to-blue-200">boost every paycheck</span>
          </motion.h1>

          <motion.p {...fadeUp(0.2)} className="text-lg text-dark-000 max-w-md mb-10 leading-relaxed">
            20,000+ brand discounts, exclusive deals, and cashback — at no cost to your company.
          </motion.p>
        ...
        <motion.div className="relative hidden lg:block h-[540px]"
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[360px] bg-white/10 backdrop-blur-xl rounded-2xl border border-white/15 p-5 shadow-2xl">
            <div className="flex items-start justify-between mb-3 pb-3 border-b border-white/10">
              <div>
                <p className="text-white text-[12px] font-bold leading-tight">Perks Store</p>
                <p className="text-dark-000 text-[10px] mt-0.5">20,000+ brands · 25+ categories</p>
              </div>
              <span className="text-[9px] font-bold text-blue-100 bg-blue-200/20 border border-blue-100/30 px-2 py-0.5 rounded-full">Live</span>
            </div>

            <div className="flex items-center gap-1.5 overflow-x-auto pb-1 mb-3">
              {CATEGORIES.map((c, i) => (
                <span key={c} className={`text-[9px] font-bold whitespace-nowrap shrink-0 px-2 py-1 rounded-full ${i === 0 ? "bg-blue-200 text-white" : "bg-white/8 text-white/70"}`}>{c}</span>
              ))}
            </div>

            <div className="grid grid-cols-3 gap-1.5 mb-3">
              {BRANDS.map((b) => (
                <div key={b.name} className="rounded-lg overflow-hidden border border-white/10 bg-white/5">
                  <div className="h-9 flex items-center justify-center text-[11px] font-extrabold text-white/95" style={{ background: b.color }}>{b.name}</div>
                  <div className="px-2 py-1.5">
                    <p className="text-[8px] uppercase tracking-[0.06em] text-white/55 font-bold truncate">{b.tag}</p>
                    {b.save && <p className="text-[10px] font-bold text-green-300">Save {b.save}</p>}
                  </div>
                </div>
              ))}
            </div>

            <div className="rounded-lg bg-green-300/15 border border-green-300/30 px-3 py-2 flex items-center justify-between">
              <span className="text-[10px] text-white/85 font-semibold">Your potential monthly savings</span>
              <span className="text-base font-bold text-green-300 tabular-nums">$240</span>
            </div>
          </div>
