...
const FLOATING_CHIPS = [
  { id: 1, emoji: "🎯", label: "Tagged to drivers",       sub: "Linked to engagement",   value: "✓",     pos: "top-[6%] right-[2%]",     delay: "0s",   val: "text-blue-200" },
  { id: 2, emoji: "📅", label: "Owners & deadlines",      sub: "No ambiguous follow-up", value: "Assign", pos: "top-[58%] right-[-2%]",   delay: "0.6s", val: "text-orange-200" },
  { id: 3, emoji: "📊", label: "Historical context",      sub: "Trended driver scores",  value: "6mo",   pos: "bottom-[10%] right-[6%]", delay: "1.1s", val: "text-green-300" },
];
...
const TASKS = [
  { task: "Address high work pressure in Design", driver: "Retention",  driverColor: "#A855F7", initials: "AD", avatarBg: "linear-gradient(135deg,#1D61F6,#6f8eff)", eta: "Jun 30", status: "ACTIVE", statusColor: "#1D61F6" },
  { task: "Run career growth workshops for Eng",  driver: "Mgmt & Ldp", driverColor: "#F59E0B", initials: "AK", avatarBg: "linear-gradient(135deg,#22C55E,#4ADE80)", eta: "Jul 15", status: "ACTIVE", statusColor: "#1D61F6" },
  { task: "Monthly D&I townhalls for alignment",  driver: "Diversity",  driverColor: "#3B82F6", initials: "AB", avatarBg: "linear-gradient(135deg,#A855F7,#C084FC)", eta: "Jun 25", status: "REVIEW", statusColor: "#F59E0B" },
  { task: "Improve manager onboarding training",  driver: "Teamwork",   driverColor: "#15803D", initials: "HR", avatarBg: "linear-gradient(135deg,#F97316,#FB923C)", eta: "Jul 1",  status: "DONE",   statusColor: "#22C55E" },
];
      ...
      <div className="relative z-10 w-full max-w-[1280px] mx-auto px-6 pt-28 pb-12 grid lg:grid-cols-[1fr_500px] gap-16 items-center">
        <div className="flex flex-col items-start">
          ...
          <motion.h1 {...fadeUp(0.1)} className="text-[46px] lg:text-[60px] font-bold text-white leading-[1.06] tracking-tight mb-6">
            <span className="block">Turn survey insights</span>
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-100 to-blue-200">into real action</span>
          </motion.h1>

          <motion.p {...fadeUp(0.2)} className="text-lg text-dark-000 max-w-md mb-10 leading-relaxed">
            Driver-tagged tasks with owners, deadlines, and tracked resolution &mdash; all inside Empuls.
          </motion.p>
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[360px] bg-white/10 backdrop-blur-xl rounded-2xl border border-white/15 p-5 shadow-2xl">
            <div className="flex items-start justify-between mb-4 pb-4 border-b border-white/10">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <p className="text-white text-[12px] font-bold leading-tight">Action Plans · Q2 Survey</p>
                  <p className="text-dark-000 text-[10px] mt-0.5">3 active · 2 in review · 5 completed</p>
                </div>
              </div>
              <span className="text-[9px] font-bold text-white px-2 py-1 rounded-full bg-blue-200">+ Create</span>
            </div>

            <div className="space-y-1.5">
              {TASKS.map((t) => (
                <div key={t.task} className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-lg px-2 py-1.5">
                  <div className="flex-1 min-w-0">
                    <p className="text-[10px] font-bold text-white truncate">{t.task}</p>
                    <div className="flex items-center gap-1.5 mt-0.5">
                      <span className="text-[8px] font-bold uppercase tracking-[0.06em] px-1.5 py-px rounded-full" style={{ background: t.driverColor + "30", color: t.driverColor }}>{t.driver}</span>
                      <span className="text-[8px] text-white/45">{t.eta}</span>
                    </div>
                  </div>
                  <div className="w-6 h-6 rounded-full flex items-center justify-center text-[8px] font-bold text-white shrink-0" style={{ background: t.avatarBg }}>{t.initials}</div>
                  <span className="text-[8px] font-bold uppercase tracking-[0.06em] px-1.5 py-0.5 rounded-full whitespace-nowrap" style={{ background: t.statusColor + "30", color: t.statusColor }}>
            ...
            <div className="pt-3 mt-3 border-t border-white/10 flex items-center justify-between">
              <span className="text-[9px] text-white/55">Q2 2024 · 247 responses · Closed Jun 30</span>
              <span className="text-[9px] font-bold text-green-300 bg-green-300/15 px-2 py-0.5 rounded-full">5 RESOLVED</span>
            </div>
          </div>
