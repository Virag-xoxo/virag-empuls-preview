...
const FLOATING_CHIPS = [
  { id: 1, emoji: "🪐", label: "8+ spending categories", sub: "Fitness · learning · more",  value: "All",   pos: "top-[6%] right-[2%]",     delay: "0s",   val: "text-blue-200" },
  { id: 2, emoji: "🌍", label: "100+ countries",         sub: "Multi-currency wallets",     value: "Global",pos: "top-[58%] right-[-2%]",   delay: "0.6s", val: "text-orange-200" },
  { id: 3, emoji: "📲", label: "Zero paperwork",         sub: "Direct redemption · receipts", value: "0",   pos: "bottom-[10%] right-[6%]", delay: "1.1s", val: "text-green-300" },
];
...
const CATEGORIES = [
  { e: "🏋️", label: "Fitness & gym",       sub: "$480 spent · 80%", color: "#22C55E", pct: 80 },
  { e: "🎓", label: "Learning & courses",   sub: "$360 spent · 60%", color: "#1D61F6", pct: 60 },
  { e: "🥗", label: "Meals & nutrition",    sub: "$648 spent · 54%", color: "#F97316", pct: 54 },
  { e: "🧠", label: "Mental wellness",      sub: "Add to wallet",     color: "#94a3b8", pct:  0 },
];
      ...
      <div className="relative z-10 w-full max-w-[1280px] mx-auto px-6 pt-28 pb-12 grid lg:grid-cols-[1fr_500px] gap-16 items-center">
        <div className="flex flex-col items-start">
          ...
          <motion.h1 {...fadeUp(0.1)} className="text-[46px] lg:text-[60px] font-bold text-white leading-[1.06] tracking-tight mb-6">
            <span className="block">Spend on what</span>
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-100 to-blue-200">actually matters</span>
          </motion.h1>

          <motion.p {...fadeUp(0.2)} className="text-lg text-dark-000 max-w-md mb-10 leading-relaxed">
            Fund a lifestyle allowance every employee spends their own way — fitness, learning, meals, family care, and more.
          </motion.p>
        ...
        <motion.div className="relative hidden lg:block h-[540px]"
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[360px] bg-white/10 backdrop-blur-xl rounded-2xl border border-white/15 p-5 shadow-2xl">
            <div className="flex items-start justify-between mb-3 pb-3 border-b border-white/10">
              <div>
                <p className="text-white text-[12px] font-bold leading-tight">My LSA wallet</p>
                <p className="text-dark-000 text-[10px] mt-0.5">FY 2025–26 · annual allowance</p>
              </div>
              <span className="text-[9px] font-bold text-orange-200 bg-orange-200/15 border border-orange-100/20 px-2 py-0.5 rounded-full whitespace-nowrap">$2,400</span>
            </div>

            <div className="mb-3">
              <div className="flex items-baseline gap-2 mb-1">
                <span className="text-3xl font-bold text-orange-200 tabular-nums">$1,488</span>
                <span className="text-[11px] text-white/55">spent · 62%</span>
              </div>
              <div className="h-2 bg-white/8 rounded-full overflow-hidden">
                <div className="h-full rounded-full" style={{ width: "62%", background: "linear-gradient(90deg,#F97316,#FB923C)" }} />
              </div>
              <p className="text-[9px] text-white/45 mt-1">$1,488 spent · $912 remaining</p>
            </div>

            <div className="space-y-1.5">
              {CATEGORIES.map((c) => (
                <div key={c.label} className="bg-white/5 border border-white/10 rounded-lg px-2.5 py-1.5">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-base shrink-0">{c.e}</span>
                    <p className="flex-1 text-[11px] font-bold text-white truncate">{c.label}</p>
                    <span className="text-[9px] text-white/55">{c.sub}</span>
                  </div>
            ...
            <div className="mt-3 pt-3 border-t border-white/10 flex items-center justify-between">
              <span className="text-[9px] text-white/55">4 more categories available</span>
              <span className="text-[9px] font-bold text-blue-100">Manage wallet →</span>
            </div>
          </div>
